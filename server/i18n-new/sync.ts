...
import { cloneDeep, without, difference, union, values } from 'lodash';
...
function generateEmptyPoFile(locale: string): POFile {
  return {
    headers: {
      language: locale,
...
export type UpdateLocalDocumentFnOptions = {
  doc: object,
  locale: string,
...
export function syncStringsWithPOFile({
  ...
  updateLocalDocumentFn: ((options: UpdateLocalDocumentFnOptions) => void),
  getTranslationForDocFn: ((doc: object, locale: string) => string),
  msgidsToDocs: { [msgid: string]: any },
  context: string,
  poFile: POFile,
}) {
  check(poFile, Object);
  ...
  check(values(msgidsToDocs), [Match.Any]);
  ...
  msgids.forEach(msgid => {
    ...
    if (poFileTranslation) {
      ...
      if (msgstr && existingTranslation !== msgstr) {
        ...
        updateLocalDocumentFn({ doc, locale, attributeDescriptor, msgstr });
      }
    } else {
      ...
      existingTranslation = existingTranslation || (isDefaultLocale ? humanize(msgid) : '');
...
function stripTranslations(poFile: POFile, context: string = '') {
  check(context, String);
  check(poFile, Object);
...
function displayStats(
  ...
  const localEmptyCount = values(msgidsToDocs)
...
function returnNullIf404(fn: Function) {
  return (...args) => {
    try {
      return fn(...args);
    } catch (error) {
...
function getSupportedLocales(resourceSlug: string, defaultLocale: string) {
...
export function syncWithTransifex({
  ...
  msgidsToDocs: { [msgid: string]: any },
  updateLocalDocumentFn: ((options: UpdateLocalDocumentFnOptions) => void),
  getTranslationForDocFn: (doc: object, attributeDescriptor: AttributeDescriptor, locale: string) => string,
}) {
  check(context, String);
